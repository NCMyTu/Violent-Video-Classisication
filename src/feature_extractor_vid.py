...
def feature_extractor():
	...
	video_list_file = args.video_name
	...
	for video_name in video_list: 
		...
		frame_path = os.path.join(temp_path, video_name.split(".")[0])
		...
		os.system(f'ffmpeg -i "{video_path}" -r 16 -q:v 2 -f image2 "{frame_path}/image_%05d.jpg"')
		...
		try:
			os.system('rmdir /s /q ' + f"\"{frame_path}\"")
			print("DONE CLEANING")
			# print(f"{frame_path}")
...
if __name__ == "__main__":
	...
	parser.add_argument('--OUTPUT_NAME', default='out.hdf5', help='The output name of the hdf5 features')
	parser.add_argument('-b', '--BATCH_SIZE', default=30, help='the batch size')
	parser.add_argument('-id', '--gpu_id', default=0, type=int)
	parser.add_argument('-p', '--video_name', help='the video name list')

	args = parser.parse_args()
	params = vars(args) # convert to ordinary dict
	print('parsed parameters:')

	OUTPUT_DIR = output_dir # params['OUTPUT_DIR']
	VIDEO_DIR = params['VIDEO_DIR']
	OUTPUT_NAME = output_name # params['OUTPUT_NAME']
